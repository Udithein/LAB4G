...
#include "Point.h"
#include "libs/stb-master/stb_easy_font.h"
...
void Button::Show() {
    ...
    glColor3f(0.25, 1, 0.70); glVertex2f(LeftTop->x, LeftTop->y);
    glColor3f(0.25, 1, 0.70); glVertex2f(RightTop->x, RightTop->y);
    glColor3f(0.25, 1, 0.70); glVertex2f(RightBottom->x, RightBottom->y);

    glColor3f(0.25, 1, 0.70); glVertex2f(RightBottom->x, RightBottom->y);
    glColor3f(0.25, 1, 0.70); glVertex2f(LeftBottom->x, LeftBottom->y);
    glColor3f(0.25, 1, 0.70); glVertex2f(LeftTop->x, LeftTop->y);
    glClear(GL_COLOR_BUFFER_BIT);
    glEnd();
    print_string(14, 5, name, 1, 0, 0);
    glPopMatrix();
}
